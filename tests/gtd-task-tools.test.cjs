...
assert.equal(url.searchParams.get('details'), parse(due).text);
const specialText = 'Позвонить & уточнить #1? https://example.com/?a=1&b=2';
const specialUrl = new URL(gtd.googleCalendarUrl(specialText + ' due:2026-12-31'));
assert.equal(specialUrl.searchParams.get('details'), parse(specialText).text);
assert.equal(specialUrl.searchParams.get('text'), parse(specialText).text);
